...
$chek = $mysqli->query("select * from tbl_rider where (mobile='".$mobile."' or email='".$mobile."') and status = 1 and password='".$password."'");
$status = $mysqli->query("select * from tbl_rider where status = 1");
if($status->num_rows !=0)
{
if($chek->num_rows != 0)
{
    $c = $mysqli->query("select * from tbl_rider where (mobile='".$mobile."' or email='".$mobile."')  and status = 1 and password='".$password."'")->fetch_assoc();
